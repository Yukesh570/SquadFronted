...
export const SubRouteEditableTable: React.FC<SubRouteEditableTableProps> = ({
  ...
  const handleTableClick = (e: React.MouseEvent) => {
    if ((e.target as HTMLElement).tagName === "DIV" && (e.target as HTMLElement).classList.contains("custom-grid-scroll")) {
  ...
  const handleInlineSave = async (id: number, field: string, newValue: string) => {
    ...
    setData((prev) => prev.map((r) => r.id === id ? { ...r, [field]: newValue } : r));
    ...
    try {
      ...
      setData((prev) => prev.map((r) => r.id === id ? { ...r, [field]: (originalRow as any)[field] } : r));
    }
  };
  ...
  if (loading) return <div className="p-20 text-center text-gray-500 animate-pulse font-medium">Fetching details...</div>;

  const statusOptions = [{ label: "Active", value: "ACTIVE" }, { label: "Inactive", value: "INACTIVE" }];
  const priorityOptions = ["1", "2", "3", "4", "5"].map(v => ({ label: v, value: v }));
      ...
      <table className="w-full text-left text-sm whitespace-nowrap border-separate border-spacing-0">
        <thead className="bg-gray-100 dark:bg-gray-800 text-text-secondary dark:text-gray-300 sticky top-0 z-20">
          <tr>
            ...
            <th className="px-4 py-2 font-bold border-b border-r dark:border-gray-600">Originating Company</th>
            <th className="px-4 py-2 font-bold border-b border-r dark:border-gray-600">Originating Client</th>
            ...
            <th className="px-4 py-2 font-bold border-b border-r dark:border-gray-600">Terminating Company</th>
            ...
            {canDelete && <th className="px-4 py-2 font-bold border-b dark:border-gray-600 text-center sticky right-0 bg-gray-100 dark:bg-gray-800 shadow-l z-30">Action</th>}
          </tr>
          ...
          <tr className="bg-gray-50 dark:bg-gray-800/80">
            ...
            <th className="p-1 border-b border-r dark:border-gray-600 font-normal"><FilterInput fieldKey="orginatingCompanyName" placeholder="Search..." value={columnFilters["orginatingCompanyName"]} onChange={handleFilterChange} minWidth="140px" /></th>
            <th className="p-1 border-b border-r dark:border-gray-600 font-normal"><FilterInput fieldKey="orginatingClientName" placeholder="Search..." value={columnFilters["orginatingClientName"]} onChange={handleFilterChange} minWidth="120px" /></th>
            ...
            <th className="p-1 border-b border-r dark:border-gray-600 font-normal"><FilterInput fieldKey="terminatingCompanyName" placeholder="Search..." value={columnFilters["terminatingCompanyName"]} onChange={handleFilterChange} minWidth="140px" /></th>
            ...
            <th className="p-1 border-b border-r dark:border-gray-600 font-normal relative z-[60]">
               <div className="w-full filter-control-wrapper" style={{ minWidth: '90px' }}>
                  <Select
                    label=""
            ...
            <th className="p-1 border-b border-r dark:border-gray-600 font-normal relative z-[60]">
               <div className="w-full filter-control-wrapper" style={{ minWidth: '130px' }}>
                 <DatePicker
                    ...
                    onChange={(date: Date | null) => handleFilterChange("createdAt", date ? formatLocalDate(date) : "")}
                 />
               </div>
            </th>
        ...
        <tbody>
          {filteredData.map((route: ExtendedCustomRouteData) => (
            <tr key={route.id} className="hover:bg-blue-50/40 dark:hover:bg-primary/5 transition-colors relative z-0 hover:z-10 focus-within:z-50 group">
              ...
              <ReadOnlyCell>{route.orginatingCompanyName || "-"}</ReadOnlyCell>
              <ReadOnlyCell>{route.orginatingClientName || "-"}</ReadOnlyCell>
              ...
              {canDelete && (
                <td className="px-4 py-2 text-center sticky right-0 bg-white dark:bg-gray-900 border-l border-b dark:border-gray-700 z-10">
                  <button onClick={() => onDelete(route.id!)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-md transition-all" title="Delete Route">
                    <Trash size={16} />
                  </button>
                </td>
              )}
          ...
          {filteredData.length === 0 && (
             <tr>
               <td colSpan={12} className="px-4 py-8 text-center text-gray-500 bg-white dark:bg-gray-900 border-b dark:border-gray-700">
