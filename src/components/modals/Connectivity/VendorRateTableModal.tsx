...
import { getVendorRateByVendorApi, type VendorRateData } from "../../../api/connectivityApi/vendorApi";

interface VendorRateTableModalProps {
  isOpen: boolean;
  onClose: () => void;
  vendor: { id: number; profileName: string } | null;
}
...
export const VendorRateTableModal: React.FC<VendorRateTableModalProps> = ({
  ...
  useEffect(() => { setCurrentPage(1); }, [vendor]);
  ...
  }, [isOpen, vendor, currentPage, rowsPerPage, apiFilters]);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Vendor Rates: ${vendor?.profileName || ""}`}
      className="max-w-4xl"
    >
          ...
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={headers.length} className="text-center py-8 text-gray-500">{Object.keys(apiFilters).length > 0 ? "No rates match your search filters." : "No rates found for this vendor."}</td></tr>
              ) : (
                rates.map((v, idx) => (
                  <tr
                    key={idx}
