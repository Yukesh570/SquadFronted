...
interface ColumnConfig extends Omit<FilterColumn, "type" | "key" | "label"> {
  ...
  label: string;
  type?: FilterColumnType;
  render?: (timezone: any) => React.ReactNode;
  options?: Option[];
  filterKey?: string;
...
const DEFAULT_TABLE_COLUMNS = ["name", "utcOffset", "abbreviation", "createdAt"];
...
const TimeZone: React.FC = () => {
  ...
  const [searchColumns, setSearchColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("timezone_search_columns");
    return saved ? JSON.parse(saved) : DEFAULT_SEARCH_COLUMNS;
  });

  useEffect(() => {
    localStorage.setItem(
      "timezone_search_columns",
  ...
  // --- Dropdown Options ---
  const [abbreviationOptions, setAbbreviationOptions] = useState<Option[]>([]);
        ...
        renderRow={(timezone, index) => (
          ...
          >
            ...
            {visibleTableFields.map((col) => (
              ...
              >
                {col.render ? col.render(timezone) : (timezone as any)[col.key] || "-"}
              </td>
            ))}
          </tr>
        )}
