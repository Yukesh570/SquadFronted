...
const CompanyCategory: React.FC = () => {
  ...
  const [searchColumns, setSearchColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("companycategory_search_columns");
    return saved ? JSON.parse(saved) : DEFAULT_SEARCH_COLUMNS;
  });

  useEffect(() => {
    localStorage.setItem(
      "companycategory_search_columns",
  ...
  return (
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      ...
      <DeleteModal
        ...
        message={`Are you sure you want to delete company category "${selectedRowCategory?.name || ""}"? This action cannot be undone.`}
      />
    </div>
  );
};
