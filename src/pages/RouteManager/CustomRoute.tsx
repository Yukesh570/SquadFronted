...
const DEFAULT_SEARCH_COLUMNS = ["routeGroup", "status", "country__name"];
const DEFAULT_TABLE_COLUMNS = ["routeGroup", "country__name", "totalSubRoutes", "status", "createdAt"];
...
const CustomRoute: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem("customroute_grouped_columns", JSON.stringify(tableColumns));
  }, [tableColumns]);
  ...
  const allColumns: ColumnConfig[] = [
    { key: "routeGroup", label: "Route Group", type: "text", filterKey: "routeGroup__icontains" },
    { key: "country__name", label: "Country", type: "text", filterKey: "country__name__icontains" },
    { key: "totalSubRoutes", label: "Sub-Routes", type: "number", filterKey: "totalSubRoutes" },
  ...
  const fetchGroupedRoutes = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      const response: any = await getGroupedCustomRoutesApi(routeName, currentPage, rowsPerPage, currentSearchParams);

      if (newController.signal.aborted) return;
      if (response && response.results) { setGroupedRoutes(response.results); setTotalItems(response.count); } 
      else if (Array.isArray(response)) { setGroupedRoutes(response); setTotalItems(response.length); } 
      else { setGroupedRoutes([]); setTotalItems(0); }
    } catch (error: any) {
  ...
  const handleSearch = () => { setCurrentPage(1); fetchGroupedRoutes(); };
  const handleClearFilters = () => { setFilterValues({}); setCurrentPage(1); fetchGroupedRoutes({}); };

  const handleAddMain = () => { if (!canCreate) return; setIsCreateModalOpen(true); };
  ...
  const menuItems: ContextMenuItem[] = selectedRowGroup ? [
    { 
      ...
      icon: <Layers size={16} />, 
      onClick: () => openSubTableModal(selectedRowGroup.routeGroup) 
    },
  ] : [];
    ...
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <h1 className="text-2xl font-semibold text-text-primary dark:text-white mr-2">Custom Route Manager</h1>
      ...
      <DataTable 
        ...
        isLoading={isLoading} 
        headerActions={canCreate ? <Button variant="primary" onClick={handleAddMain} leftIcon={<Plus size={18} />}>Create Route</Button> : null}
        renderRow={(routeGroupObj, index) => (
          <tr 
            key={index}
            onContextMenu={(e) => handleContextMenu(e, routeGroupObj)}
            onDoubleClick={() => openSubTableModal(routeGroupObj.routeGroup)} 
              ...
              if (col.key === "status") {
                return (
                  <td key={col.key} className="px-4 py-4 text-sm">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${routeGroupObj.status === "ACTIVE" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"}`}>{routeGroupObj.status}</span>
                  </td>
                );
              }
              if (col.key === "routeGroup") {
                return <td key={col.key} className="px-4 py-4 text-sm font-semibold text-primary">{cellData || "-"}</td>;
              }
