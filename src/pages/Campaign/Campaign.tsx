...
const CreateCampaignForm: React.FC = () => {
  const [formData, setFormData] = useState<
    Omit<CampaignFormData, "scheduleDateTime">
  >({
    campaignName: "",
    objective: "",
  ...
  useEffect(() => {
    console.log("Template changed:", formData.template);
    console.log("Template content changed:", formData.content);
  }, [formData.template, formData.content]);
  ...
  const handleSelectChange = (
    ...
    console.log("Template content:", formData.content);
  };

  /*************  ✨ Windsurf Command ⭐  *************/
  /**
   * Handle changes to the audience type
   * When the audience type changes to "import", the contact number is reset to an empty string
   * When the audience type changes to "specify", the csv file is reset to null
   * @param {string} value - The new audience type
   */
  /*******  2c4a391b-32be-488f-a0f8-9abd57b80c9f  *******/
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.campaignName || !formData.content) {
        throw new Error("Campaign Name and Content are required fields.");
      }
      if (!formData.template) {
        throw new Error("Template is required.");
      }
      if (formData.audienceType === "specify" && !formData.contactNumber) {
      ...
      if (formData.scheduleType === "datetime" && !scheduleDate) {
        throw new Error("A schedule date and time is required.");
      }
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <>
            ...
            <form
              ...
              className="bg-white dark:bg-gray-800 rounded-lg space-y-1 "
            >
              <label className="text-xs font-medium text-text-secondary mb-1 mt-[-4px]">
                Content
              </label>
