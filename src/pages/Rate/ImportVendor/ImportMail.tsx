...
import { getImportMailsApi, type ImportMailData } from "../../../api/rateApi/ImportVendor/importMailApi";
import { getVendorsApi } from "../../../api/connectivityApi/vendorApi";
import { ImportMailModal } from "../../../components/modals/Rate/ImportVendor/ImportMailModal";
...
interface ColumnConfig extends Omit<FilterColumn, 'type' | 'key' | 'label'> {
  ...
  label: string;
  type?: FilterColumnType;
  render?: (data: ImportMailData) => React.ReactNode;
  options?: Option[];
  filterKey?: string;
...
const ImportMail: React.FC = () => {
  const [data, setData] = useState<ImportMailData[]>([]);
  ...
  const [editingData, setEditingData] = useState<ImportMailData | null>(null);
  const [isViewMode, setIsViewMode] = useState(false);

  // Context Menu
  const [contextMenuPos, setContextMenuPos] = useState<{ x: number; y: number } | null>(null);
  const [selectedRowData, setSelectedRowData] = useState<ImportMailData | null>(null);

  // Filters & Pagination
  const [searchColumns, setSearchColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("import_mail_search_columns");
    return saved ? JSON.parse(saved) : DEFAULT_SEARCH_COLUMNS;
  });

  useEffect(() => {
    localStorage.setItem(
      "import_mail_search_columns",
  ...
  useEffect(() => {
    localStorage.setItem("import_mail_table_columns", JSON.stringify(tableColumns));
  }, [tableColumns]);
  ...
  const fetchData = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      const response: any = await getImportMailsApi(routeName, currentPage, rowsPerPage, currentSearchParams);
      ...
      if (error.name !== "AbortError") toast.error("Failed to fetch import mails.");
    } finally {
  ...
  const handleEdit = (item: ImportMailData) => { setEditingData(item); setIsViewMode(false); setIsModalOpen(true); };
  const handleView = (item: ImportMailData) => { setEditingData(item); setIsViewMode(true); setIsModalOpen(true); };

  const handleContextMenu = (e: React.MouseEvent, item: ImportMailData) => {
    e.preventDefault();
    ...
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 text-sm text-text-secondary">
          ...
          <span>/</span><span className="text-text-primary dark:text-white">Import Mail</span>
        </div>
      </div>
