...
interface ColumnConfig extends Omit<FilterColumn, 'type' | 'key' | 'label'> {
  ...
  label: string;
  type?: FilterColumnType;
  render?: (data: ImportAttachmentData) => React.ReactNode;
  options?: Option[];
  filterKey?: string;
...
const DEFAULT_SEARCH_COLUMNS = ["fileName", "parseStatus"];
const DEFAULT_TABLE_COLUMNS = ["fileName", "fileType", "parseStatus", "createdAt"];

const ImportAttachment: React.FC = () => {
  const [data, setData] = useState<ImportAttachmentData[]>([]);
  const [totalItems, setTotalItems] = useState(0);
  const [isLoading, setIsLoading] = useState(true);

  // Modals
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingData, setEditingData] = useState<ImportAttachmentData | null>(null);
  const [isViewMode, setIsViewMode] = useState(false);

  // Context Menu
  const [contextMenuPos, setContextMenuPos] = useState<{ x: number; y: number } | null>(null);
  const [selectedRowData, setSelectedRowData] = useState<ImportAttachmentData | null>(null);

  // Filters & Pagination
  const [searchColumns, setSearchColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("import_attachment_search_columns");
    return saved ? JSON.parse(saved) : DEFAULT_SEARCH_COLUMNS;
  });

  useEffect(() => {
    localStorage.setItem(
      "import_attachment_search_columns",
  ...
  const [tableColumns, setTableColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("import_attachment_table_columns");
  ...
  useEffect(() => {
    ...
    const moduleLabel = activeItem?.innerText?.split("\n")[0].trim() || "Import Attachment";
    actionHelper(moduleLabel, `Opened ${moduleLabel} Module`, false);
  }, []);

  const parseStatusOptions: Option[] = [
  ...
  const allColumns: ColumnConfig[] = [
    ...
    { key: "fileType", label: "File Type", type: "text", filterKey: "fileType__icontains" },
    { key: "fileHash", label: "File Hash", type: "text", filterKey: "fileHash__icontains" },
    { 
      key: "messageId", 
      label: "Message ID", 
      type: "text", 
  ...
  const fetchData = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      const response: any = await getImportAttachmentsApi(routeName, currentPage, rowsPerPage, currentSearchParams);
      ...
      if (error.name !== "AbortError") toast.error("Failed to fetch import attachments.");
    } finally {
  ...
  const handleEdit = (item: ImportAttachmentData) => { setEditingData(item); setIsViewMode(false); setIsModalOpen(true); };
  const handleView = (item: ImportAttachmentData) => { setEditingData(item); setIsViewMode(true); setIsModalOpen(true); };

  const handleContextMenu = (e: React.MouseEvent, item: ImportAttachmentData) => {
    e.preventDefault();
    ...
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <h1 className="text-2xl font-semibold text-text-primary dark:text-white mr-2">Import Attachments</h1>
        ...
        <div className="flex items-center space-x-2 text-sm text-text-secondary">
          ...
          <span>/</span><span className="text-text-primary dark:text-white">Import Attachments</span>
        </div>
      </div>
      ...
      <ImportAttachmentModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={fetchData} moduleName={routeName} editingData={editingData} isViewMode={isViewMode} />
    </div>
  );
};
