...
import { VendorRateTableModal } from "../../components/modals/Rate/VendorRateTableModal";
...
const GroupModal = ({ isOpen, onClose, onSuccess, moduleName, editingGroup }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingGroup) {
        await updateVendorRateGroupApi(editingGroup.id, formData, moduleName);
        toast.success("Group updated!");
      } else {
        await createVendorRateGroupApi(formData, moduleName);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editingGroup ? "Edit Vendor Rate Group" : "Create Vendor Rate Group"} className="max-w-md">
...
const VendorRate: React.FC = () => {
  ...
  const [groupedRates, setGroupedRates] = useState<VendorRateGroupData[]>([]);
  ...
  const [tableColumns, setTableColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("vendorrate_grouped_columns");
    return saved ? JSON.parse(saved) : DEFAULT_TABLE_COLUMNS;
  });

  useEffect(() => {
    localStorage.setItem("vendorrate_grouped_columns", JSON.stringify(tableColumns));
  }, [tableColumns]);
  ...
  const routeName = location.pathname.split("/")[1] || "vendorRate";
  const abortControllerRef = useRef<AbortController | null>(null);

  const hasLoggedOpening = useRef(false);
  useEffect(() => {
    if (!hasLoggedOpening.current) {
      setTimeout(() => { actionHelper("Vendor Rate", `Opened Vendor Rate Module`, false); }, 100);
  ...
  const fetchGroupedRates = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      if (error.name !== "AbortError") { toast.error("Failed to fetch vendor rate groups."); }
    } finally {
  ...
  const handleDelete = async () => {
    if (deleteId && canDelete) {
      try {
        await deleteVendorRateGroupApi(deleteId, routeName);
