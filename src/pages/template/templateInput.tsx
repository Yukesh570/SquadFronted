...
const DEFAULT_TABLE_COLUMNS = ["name", "content", "createdAt"];
...
const CampaignTemplatePage: React.FC = () => {
  ...
  const [searchColumns, setSearchColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("campaigntemplate_search_columns");
    return saved ? JSON.parse(saved) : DEFAULT_SEARCH_COLUMNS;
  });

  useEffect(() => {
    localStorage.setItem(
      "campaigntemplate_search_columns",
  ...
  const fetchTemplates = async (overrideParams?: Record<string, string>) => {
    ...
    try {
      ...
      const response: any = await getTemplatesApi(
        currentPage,
        rowsPerPage,
        currentSearchParams
      );
