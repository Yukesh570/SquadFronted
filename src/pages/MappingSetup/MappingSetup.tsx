...
const DEFAULT_TABLE_COLUMNS = [
  "ratePlan", "country", "countryCode", "timeZone", "network", 
  "MCC", "MNC", "rate", "createdAt"
];
...
const MappingSetup: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem("mappingsetup_table_columns", JSON.stringify(tableColumns));
  }, [tableColumns]);
  ...
  const allColumns: ColumnConfig[] = [
    ...
    { key: "country", label: "Country", type: "text", filterKey: "country__icontains" },
    { key: "countryCode", label: "Country Code", type: "text", filterKey: "countryCode__icontains" },
    { key: "timeZone", label: "Time Zone", type: "text", filterKey: "timeZone__icontains" },
    ...
    { key: "dateTime", label: "Date Time", type: "text", filterKey: "dateTime__icontains" },
  ...
  const fetchMappings = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      const response = await getMappingSetupsApi(routeName, currentPage, rowsPerPage, currentSearchParams);
  ...
  const handleSearch = () => { setCurrentPage(1); fetchMappings(); };
  const handleClearFilters = () => { setFilterValues({}); setCurrentPage(1); fetchMappings({}); };
  ...
  const handleDelete = async () => {
    if (deleteId && canDelete) {
      try {
        ...
        fetchMappings();
      } catch (error) { toast.error("Failed to delete mapping."); }
      setDeleteId(null);
    }
  };
    ...
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <h1 className="text-2xl font-semibold text-text-primary dark:text-white mr-2">Mapping Setup</h1>
        ...
        <div className="flex items-center space-x-2 text-sm text-text-secondary">
          ...
          <span>/</span><span className="text-text-primary dark:text-white">Mapping Setup</span>
        </div>
      </div>
      ...
      <MappingSetupModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={fetchMappings} moduleName={routeName} editingMapping={editingMapping} isViewMode={isViewMode} />
      <DeleteModal isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Mapping Setup" message="Are you sure you want to delete this mapping setup? This action cannot be undone." />
    </div>
  );
};
