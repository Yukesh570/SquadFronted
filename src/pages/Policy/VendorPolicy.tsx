...
const VendorPolicy: React.FC = () => {
  ...
  const [editingPolicy, setEditingPolicy] = useState<VendorPolicyData | null>(
    null,
  );
  ...
  const [selectedRowPolicy, setSelectedRowPolicy] =
    useState<VendorPolicyData | null>(null);
  ...
  const fetchPolicies = async (
    ...
    try {
      ...
      const response: any = await getVendorPoliciesApi(
        currentPage,
        rowsPerPage,
      );
  ...
  const handleEdit = (policy: VendorPolicyData) => {
    if (!canUpdate) return;
  ...
  const handleView = (policy: VendorPolicyData) => {
    setEditingPolicy(policy);
    setIsViewMode(true);
    setIsModalOpen(true);
  };
          ...
          >
            ...
            {visibleTableFields.map((col) => {
              ...
              return (
                <td
                  key={col.key}
                  className={`px-4 py-4 text-sm text-text-secondary dark:text-gray-300 whitespace-nowrap ${col.key === "vendor_name" ? "font-medium text-text-primary dark:text-white" : ""}`}
                >
                  {cellData ?? "-"}
                </td>
              );
