...
const ClientPolicy: React.FC = () => {
  ...
  const [editingPolicy, setEditingPolicy] = useState<ClientPolicyData | null>(
    null,
  );
  ...
  const [selectedRowPolicy, setSelectedRowPolicy] =
    useState<ClientPolicyData | null>(null);
  ...
  const fetchPolicies = async (
    ...
    try {
        ...
        if (value) {
          ...
          if (columnDef?.options && columnDef.filterKey) {
            ...
            const isNameField =
              ...
              columnDef.filterKey?.includes("__icontains");
      ...
      const response: any = await getClientPoliciesApi(
        currentPage,
        rowsPerPage,
      );
  ...
  const handleEdit = (policy: ClientPolicyData) => {
    if (!canUpdate) return;
  ...
  const handleView = (policy: ClientPolicyData) => {
    setEditingPolicy(policy);
    setIsViewMode(true);
    setIsModalOpen(true);
  };
    ...
    ? [
        {
          label: "View Details",
          icon: <Eye size={16} />,
          onClick: () => handleView(selectedRowPolicy),
        },
        ...(canUpdate
          ? [
              {
                label: "Edit Policy",
                icon: <Edit size={16} />,
                onClick: () => handleEdit(selectedRowPolicy),
              },
            ]
          : []),
        ...(canDelete
          ? [
              {
                ...
                onClick: () => setDeleteId(selectedRowPolicy.id!),
              },
            ]
          : []),
      ]
          ...
          >
            ...
            {visibleTableFields.map((col) => {
              ...
              return (
                <td
                  key={col.key}
                  className={`px-4 py-4 text-sm text-text-secondary dark:text-gray-300 whitespace-nowrap ${col.key === "client_name" ? "font-medium text-text-primary dark:text-white" : ""}`}
                >
                  {cellData ?? "-"}
                </td>
              );
