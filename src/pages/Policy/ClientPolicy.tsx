...
import {
  ...
  deleteClientPolicyApi,
  type ClientPolicyData,
} from "../../api/policyApi/clientPolicyApi";
import { getClientsApi } from "../../api/clientApi/clientApi";
import { ClientPolicyModal } from "../../components/modals/Policy/ClientPolicyModal";
...
interface ColumnConfig extends FilterColumn {
  render?: (data: ClientPolicyData) => React.ReactNode;
  options?: Option[];
  filterKey?: string;
...
const DEFAULT_SEARCH_COLUMNS = ["client_name"];
const DEFAULT_TABLE_COLUMNS = ["client_name", "maxTps", "maxSessions", "senderIdPolicy", "idleTimeoutSec"];

const ClientPolicy: React.FC = () => {
  const { canCreate, canUpdate, canDelete } = usePagePermissions();
  const [policies, setPolicies] = useState<ClientPolicyData[]>([]);
  ...
  const [editingPolicy, setEditingPolicy] = useState<ClientPolicyData | null>(null);
  const [deleteId, setDeleteId] = useState<number | null>(null);
  const [isViewMode, setIsViewMode] = useState(false);

  const [contextMenuPos, setContextMenuPos] = useState<{ x: number; y: number } | null>(null);
  const [selectedRowPolicy, setSelectedRowPolicy] = useState<ClientPolicyData | null>(null);

  const [searchColumns, setSearchColumns] = useState<string[]>(DEFAULT_SEARCH_COLUMNS);
  const [filterValues, setFilterValues] = useState<Record<string, string>>({});

  const [tableColumns, setTableColumns] = useState<string[]>(() => {
    const saved = localStorage.getItem("clientpolicy_table_columns");
    return saved ? JSON.parse(saved) : DEFAULT_TABLE_COLUMNS;
  });

  useEffect(() => {
    localStorage.setItem("clientpolicy_table_columns", JSON.stringify(tableColumns));
  }, [tableColumns]);
  ...
  const routeName = location.pathname.split("/").pop() || "clientPolicy";
  ...
  const allColumns: ColumnConfig[] = [
    { key: "client_name", label: "Client Name", type: "text", options: clientOptions, filterKey: "client__name__icontains" },
  ...
  const fetchPolicies = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      searchColumns.forEach((key) => {
        ...
        if (value) {
          ...
          if (columnDef?.options && columnDef.filterKey) {
            ...
            const isNameField = columnDef.filterKey?.includes("__name") || columnDef.filterKey?.includes("__icontains");
      ...
      const response: any = await getClientPoliciesApi(routeName, currentPage, rowsPerPage, currentSearchParams);
  ...
  const handleDelete = async () => {
    if (deleteId && canDelete) {
      try {
        await deleteClientPolicyApi(deleteId, routeName);
        toast.success("Client policy deleted.");
        fetchPolicies();
  ...
  const handleEdit = (policy: ClientPolicyData) => { if (!canUpdate) return; setEditingPolicy(policy); setIsViewMode(false); setIsModalOpen(true); };
  const handleAdd = () => { if (!canCreate) return; setEditingPolicy(null); setIsViewMode(false); setIsModalOpen(true); };
  const handleView = (policy: ClientPolicyData) => { setEditingPolicy(policy); setIsViewMode(true); setIsModalOpen(true); };

  const handleContextMenu = (e: React.MouseEvent, item: ClientPolicyData) => {
    e.preventDefault();
    ...
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <h1 className="text-2xl font-semibold text-text-primary dark:text-white mr-2">Client Policies</h1>
        ...
        <div className="flex items-center space-x-2 text-sm text-text-secondary">
          ...
          <span>/</span><span className="text-text-primary dark:text-white">Client Policy</span>
        </div>
      </div>
        ...
        renderRow={(item, index) => (
          <tr key={item.id || index} onContextMenu={(e) => handleContextMenu(e, item)} className="hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-200 dark:border-gray-700 cursor-context-menu transition-colors">
            ...
            {visibleTableFields.map((col) => {
              ...
              return <td key={col.key} className={`px-4 py-4 text-sm text-text-secondary dark:text-gray-300 whitespace-nowrap ${col.key === "client_name" ? "font-medium text-text-primary dark:text-white" : ""}`}>{cellData ?? "-"}</td>;
            })}
          </tr>
        )}
      />
      ...
      <ClientPolicyModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={fetchPolicies} moduleName={routeName} editingPolicy={editingPolicy} isViewMode={isViewMode} />
      <DeleteModal isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Client Policy" message="Are you sure you want to delete this policy?" />
    </div>
  );
};
