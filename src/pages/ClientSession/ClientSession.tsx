...
const DEFAULT_TABLE_COLUMNS = ["sessionId", "clientUsername", "companyName", "systemId", "remoteIp", "status", "connectedAt"];
...
const ClientSession: React.FC = () => {
  ...
  const [selectedSession, setSelectedSession] = useState<ClientSessionData | null>(null);
  ...
  useEffect(() => {
    localStorage.setItem("clientsession_table_columns", JSON.stringify(tableColumns));
  }, [tableColumns]);
  ...
  const renderStatusBadge = (status: string) => {
    const isActive = status?.toUpperCase() === "BOUND" || status?.toUpperCase() === "CONNECTED";
    return (
  ...
  const allColumns: ColumnConfig[] = [
    { key: "sessionId", label: "Session ID", type: "text", filterKey: "sessionId__icontains" },
    { key: "clientUsername", label: "Client Username", type: "text", filterKey: "client__name__icontains" },
    { key: "companyName", label: "Company Name", type: "text" },
    { key: "systemId", label: "System ID", type: "text", filterKey: "systemId__icontains" },
    { key: "bindType", label: "Bind Type", type: "text", filterKey: "bindType__icontains" },
    { key: "remoteIp", label: "Remote IP", type: "text", filterKey: "remoteIp__icontains" },
    { key: "remotePort", label: "Remote Port", type: "number", filterKey: "remotePort" },
    { key: "status", label: "Status", type: "text", filterKey: "status__icontains", render: (c) => renderStatusBadge(c.status) },

    // Date Fields mapped dynamically 
    { key: "connectedAt", label: "Connected At (Exact)", tableLabel: "Connected At", type: "date", filterKey: "connectedAt", render: (c) => (c.connectedAt ? new Date(c.connectedAt).toLocaleString() : "-") },
    { key: "connectedAt__range", label: "Connected At (Range)", type: "date_range", filterKey: "connectedAt", isSearchOnly: true },
    { key: "connectedAt__gt_lt", label: "Connected At (After/Before)", type: "date_gt_lt", filterKey: "connectedAt", isSearchOnly: true },

    { key: "boundAt", label: "Bound At (Exact)", tableLabel: "Bound At", type: "date", filterKey: "boundAt", render: (c) => (c.boundAt ? new Date(c.boundAt).toLocaleString() : "-") },
    { key: "boundAt__range", label: "Bound At (Range)", type: "date_range", filterKey: "boundAt", isSearchOnly: true },
    { key: "boundAt__gt_lt", label: "Bound At (After/Before)", type: "date_gt_lt", filterKey: "boundAt", isSearchOnly: true },

    { key: "last_activityAt", label: "Last Activity (Exact)", tableLabel: "Last Activity", type: "date", filterKey: "last_activityAt", render: (c) => (c.last_activityAt ? new Date(c.last_activityAt).toLocaleString() : "-") },
    { key: "last_activityAt__range", label: "Last Activity (Range)", type: "date_range", filterKey: "last_activityAt", isSearchOnly: true },
    { key: "last_activityAt__gt_lt", label: "Last Activity (After/Before)", type: "date_gt_lt", filterKey: "last_activityAt", isSearchOnly: true },
  ];
  ...
  const fetchData = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      const response: any = await getClientSessionsApi(routeName, currentPage, rowsPerPage, currentSearchParams);

      if (newController.signal.aborted) return;
      if (response && response.results) { 
        // FIX: Map sessionId to id to satisfy DataTable
        const mappedList = response.results.map((item: any) => ({ ...item, id: item.sessionId }));
        setData(mappedList); 
        setTotalItems(response.count); 
      } 
      else if (Array.isArray(response)) { 
        const mappedList = response.map((item: any) => ({ ...item, id: item.sessionId }));
        setData(mappedList); 
      ...
      if (error.name !== "AbortError") toast.error("Failed to fetch client sessions.");
    } finally {
  ...
  const handleView = (session: ClientSessionData) => { setSelectedSession(session); setIsModalOpen(true); };
  ...
  const menuItems: ContextMenuItem[] = selectedSession ? [
    { label: "View Details", icon: <Eye size={16} />, onClick: () => handleView(selectedSession) }
  ] : [];
    ...
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <h1 className="text-2xl font-semibold text-text-primary dark:text-white mr-2">Client Sessions</h1>
        ...
        <div className="flex items-center space-x-2 text-sm text-text-secondary">
          ...
          <span>/</span><span className="text-text-primary dark:text-white">Client Sessions</span>
        </div>
      </div>
      ...
      <DataTable serverSide={true} data={data} totalItems={totalItems} currentPage={currentPage} rowsPerPage={rowsPerPage} onPageChange={setCurrentPage} onRowsPerPageChange={setRowsPerPage} headers={tableHeaders} isLoading={isLoading}
        renderRow={(item, index) => (
          <tr key={item.sessionId || index} onContextMenu={(e) => handleContextMenu(e, item)} className="hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-200 dark:border-gray-700 cursor-context-menu transition-colors">
            ...
            {visibleTableFields.map((col) => {
              ...
              return <td key={col.key} className={`px-4 py-4 text-sm text-text-secondary dark:text-gray-300 whitespace-nowrap ${col.key === "sessionId" ? "font-medium text-text-primary dark:text-white" : ""}`}>{cellData || "-"}</td>;
            })}
          </tr>
        )}
      />
      ...
      <ClientSessionModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} sessionData={selectedSession} />
    </div>
  );
};
