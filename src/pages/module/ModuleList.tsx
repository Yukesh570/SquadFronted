...
import { getSideBarApi, deleteSideBarApi, type SideBarApi } from "../../api/sidebarApi/sideBarApi";
...
const ModuleList: React.FC = () => {
  ...
  const headers = ["S.N.", "Label", "URL", "Icon", "Order", "Status", "Actions"];
  ...
  return (
    <div className="container mx-auto">
      ...
      <FilterCard
        ...
        onClear={() => { setLabelFilter(""); setUrlFilter(""); }}
      >
        <Input 
      ...
      <DataTable 
        ...
        renderRow={(module, index) => (
          <tr key={module.id || index} className="hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-200 dark:border-gray-700">
             <td className="px-4 py-4 text-sm text-text-primary dark:text-white">
               {index + 1}
             </td>
             <td className="px-4 py-4 text-sm text-text-primary dark:text-white font-medium">{module.label}</td>
             <td className="px-4 py-4 text-sm text-text-primary dark:text-white">/{module.url}</td>
             <td className="px-4 py-4 text-sm text-text-primary dark:text-white">{module.icon}</td>
             <td className="px-4 py-4 text-sm text-text-primary dark:text-white">{module.order}</td>
             ...
             <td className="px-4 py-4 text-sm">
                <div className="flex items-center space-x-2">
                  <Button variant="secondary" size="xs" onClick={() => handleEdit(module)}>
                      <Edit size={14}/>
                  </Button>
                  <Button variant="danger" size="xs" onClick={() => setDeleteId(module.id!)}>
                      <Trash size={14}/>
                  </Button>
                </div>
             </td>
      ...
      <ModuleModal 
        ...
        onSuccess={() => {
          fetchModules();
          refreshNavItems(); // Refresh sidebar when Adding/Editing too
        }}
