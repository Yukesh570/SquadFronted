...
const OperatorNetworkCode: React.FC = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<OperatorNetworkCodeData | null>(null);
  ...
  useEffect(() => {
    localStorage.setItem("operatornetworkcode_table_columns", JSON.stringify(tableColumns));
  }, [tableColumns]);
  ...
  useEffect(() => {
    getCountriesApi("country", 1, 1000).then((res: any) => {
      ...
      const options: Option[] = list.map((c: any) => ({ label: c.name, value: String(c.id) }));
      setCountryOptions(options.sort((a, b) => a.label.localeCompare(b.label)));
    }).catch(console.error);

    getOperatorsApi("operator", 1, 1000).then((res: any) => {
      const list = res.results || (Array.isArray(res) ? res : []);
      const options: Option[] = list.map((o: any) => ({ label: o.name || o.operator_name, value: String(o.id) }));
  ...
  const networkTypeOptions: Option[] = [
    { label: "GSM", value: "GSM" }, { label: "LTE", value: "LTE" },
    { label: "5G", value: "5G" }, { label: "CDMA", value: "CDMA" },
  ...
  const allColumns: ColumnConfig[] = [
    { key: "networkName", label: "Network Name", type: "text", filterKey: "networkName__icontains" },
    { key: "operator_name", label: "Operator", type: "text", options: operatorOptions, filterKey: "operator__name__icontains" },
    { key: "country_name", label: "Country", type: "text", options: countryOptions, filterKey: "country__name__icontains" },
    
    { key: "MCC", label: "MCC", type: "text", filterKey: "MCC__icontains" },
    { key: "MNC", label: "MNC", type: "text", filterKey: "MNC__icontains" },
    
    { key: "networkType", label: "Network Type", type: "text", options: networkTypeOptions, filterKey: "networkType__icontains" },
    { key: "status", label: "Status", type: "text", options: statusOptions, filterKey: "status__icontains", render: (c) => renderBadge(c.status) },
    { key: "isPrimary", label: "Is Primary", type: "boolean", options: booleanOptions, filterKey: "isPrimary__icontains", render: (c) => renderBadge(c.isPrimary) },

    { key: "effectiveFrom", label: "Effective From (Exact)", tableLabel: "Effective From", type: "date", filterKey: "effectiveFrom" },
  ...
  const fetchData = async (filters: Record<string, string> | null = null) => {
    ...
    try {
      ...
      searchColumns.forEach((key) => {
        ...
        if (value) {
          ...
          if (columnDef?.options) {
            ...
            if (start && end) currentSearchParams[`${baseKey}__range`] = `${start},${end}`;
            else {
      ...
      const response: any = await getOperatorNetworkCodesApi(routeName, currentPage, rowsPerPage, currentSearchParams);

      if (newController.signal.aborted) return;
      if (response && response.results) { setData(response.results); setTotalItems(response.count); } 
      else if (Array.isArray(response)) { setData(response); setTotalItems(response.length); } 
      else { setData([]); setTotalItems(0); }
    } catch (error: any) {
      if (error.name !== "AbortError") toast.error("Failed to fetch network codes.");
    } finally {
  ...
  const handleDelete = async () => {
    if (deleteId && canDelete) {
      try {
        ...
        fetchData();
      } catch (error) { toast.error("Failed to delete network code."); }
      setDeleteId(null);
    }
  };

  const handleEdit = (item: OperatorNetworkCodeData) => { if (!canUpdate) return; setEditingData(item); setIsViewMode(false); setIsModalOpen(true); };
  const handleAdd = () => { if (!canCreate) return; setEditingData(null); setIsViewMode(false); setIsModalOpen(true); };
  const handleView = (item: OperatorNetworkCodeData) => { setEditingData(item); setIsViewMode(true); setIsModalOpen(true); };

  const handleContextMenu = (e: React.MouseEvent, item: OperatorNetworkCodeData) => {
    e.preventDefault();
  ...
  const menuItems: ContextMenuItem[] = selectedRow ? [
    ...
    ...(canUpdate ? [{ label: "Edit Info", icon: <Edit size={16} />, onClick: () => handleEdit(selectedRow) }] : []),
    ...(canDelete ? [{ label: "Delete", icon: <Trash size={16} />, variant: "danger" as const, onClick: () => setDeleteId(selectedRow.id!) }] : []),
  ] : [];
    ...
    <div className="container mx-auto" onClick={() => setContextMenuPos(null)}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <h1 className="text-2xl font-semibold text-text-primary dark:text-white mr-2">Operator Network Codes</h1>
        ...
        <div className="flex items-center space-x-2 text-sm text-text-secondary">
          ...
          <span>/</span><span className="text-text-primary dark:text-white">Network Codes</span>
        </div>
      </div>
